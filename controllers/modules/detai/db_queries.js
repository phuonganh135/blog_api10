...
module.exports.getDeTaiLuanVanSVDXList = function (gv_ma, callback) {
    pool.query('SELECT * FROM detai WHERE gv_ma = $1 and dt_xoa = 0 and dt_trangthai=\'svdx\' and ldt_ma=\'lv\'',[gv_ma], async function (error, results) {
        var listDeTai = results.rows;
        // var listResult = results.rows;
        for (var i = 0; i < listDeTai.length; i++) {
            var deTai = listDeTai[i];
...
module.exports.getDeTaiNienLuanSVDXList = function (gv_ma, callback) {
    pool.query('SELECT * FROM detai WHERE gv_ma = $1 and dt_xoa = 0 and dt_trangthai=\'svdx\' and ldt_ma=\'nl\'',[gv_ma], async function (error, results) {
        var listDeTai = results.rows;
        // var listResult = results.rows;
        for (var i = 0; i < listDeTai.length; i++) {
            var deTai = listDeTai[i];
...
module.exports.getDeTaiAndSinhVienInfo = function (sv_ma, dt_ma, callback) {
    pool.query('SELECT * FROM detai as dt , sinhvien_dk_detai as svdkdt , sinhvien as sv , trangthai_dangki as ttdk WHERE svdkdt.sv_ma = sv.sv_ma and svdkdt.dt_ma = dt.dt_ma and svdkdt.ttdk_ma = ttdk.ttdk_ma and svdkdt.sv_ma=$1 and dt.dt_ma = $2 and dt.dt_xoa = 0 and sv.sv_xoa=0 and svdkdt.svdkdt_xoa=0 and ttdk.ttdk_xoa=0 and ( svdkdt.ttdk_ma=\'cn\' or svdkdt.ttdk_ma=\'cnbc\' or svdkdt.ttdk_ma=\'tcbc\'  )', [sv_ma, dt_ma], (error, results) => {
        callback(error, results.rows);
    });
};
